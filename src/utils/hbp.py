...
class SourceKeyPoints(nn.Module):
    def __init__(self, bm: Union[str, BodyModel], num_betas: int = 16, joint_idx: list = [i for i in range(21)]):
...
def transform_smpl_coordinate(
    bm_fname: Path, trans: np.ndarray, root_orient: np.ndarray, betas: np.ndarray, rotxyz: Union[np.ndarray, List]
) -> Dict:
    """
...
def run_ik_engine(res_path, motion, batch_size, smpl_path, vposer_dir, num_betas, device, verbosity):
    ...
    transformed_d = transform_smpl_coordinate(
        bm_fname=smpl_path, trans=d["trans"], root_orient=d["root_orient"], betas=d["betas"], rotxyz=[90, 0, 0]
    )
...
def make_vids(vid_path, smpl_data, len_motion, smpl_path, num_betas, fps, rotate=True):
    ...
    mean_pose_hand = np.repeat(
        np.concatenate([smpl_dict["hands_meanl"], smpl_dict["hands_meanr"]])[None], axis=0, repeats=len_motion
    )
