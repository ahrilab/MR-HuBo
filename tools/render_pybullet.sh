...
robot_name=$1
valid_robot_names=("COMAN" "NAO")

if [[ ! " ${valid_robot_names[@]} " =~ " ${robot_name} " ]]; then
    echo "Invalid robot name. Please choose between 'COMAN' and 'NAO'."
    exit 1
fi
...
output_ext=$2
input_dir="out/pred_motions/$robot_name"
...
do
    ...
    python src/visualize/pybullet_render.py -r $robot_name -v front --fps 120 -s -rp $input_dir/$filename.pkl -op $output_dir/$filename.$output_ext
done
